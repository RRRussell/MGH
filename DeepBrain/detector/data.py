...
# This class preprocesses the original CT images and get the format required by the detector network.
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None):
        ...
        if self.phase != 'test':
            ...
            for i, l in enumerate(labels):
                if len(l) > 0 :
                    for t in l:
                        # The larger the aneurysm, the more times we train.
                        if t[3]>sizelim:
    ...
    # This function will be called when enumerating the data_loader.
    def __getitem__(self, idx,split=None):

        t = time.time()
        np.random.seed(int(str(t%1)[2:7]))# seed according to time
        
        # If it is in the testing phase, you do not need to generate random images, 
        # otherwise you have to generate it in a certain proportion.
        isRandomImg  = False
        if self.phase !='test':
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                # Extract small cubes and corresponding parameters near the aneurysm.
                sample, target, bboxes, coord = self.crop(imgs, bbox[1:], bboxes,isScale,isRandom)
                # Do normalization.
                sample = (sample.astype(np.float32)+1000)/2000
                # np.save("./crop/"+ str(idx) + "crop.npy",sample)
                # np.save("./crop/"+ str(idx) + "target.npy",target)

                # Image enhancement was performed on the positive samples.
                # Details are too troublesome, so I did not study them carefully.
                ...
                isScale = self.augtype['scale'] and (self.phase=='train')
                # Randomly select a graph for training without label
                sample, target, bboxes, coord = self.crop(imgs, [], bboxes,isScale=False,isRand=True)
                sample = (sample.astype(np.float32)+1000)/2000
            
            # We get (xa,ya,za,da), which are the coordinates and scale for the anchor i from label_mapping.
            ...
            # For testing, we need to cut the original CT image and then generate several cubes.
            ...
            # Check if the small cube we segmented is correct.
            # from PIL import Image
            # import scipy.misc
            # for ii in range(len(imgs)):
            #     test_clean = imgs[ii][0][47]
            #     scipy.misc.imsave("./test_splits/"+str(ii)+".png",test_clean)
            ...
            imgs = (imgs.astype(np.float32)+1000)/2000

            return torch.from_numpy(imgs), bboxes, torch.from_numpy(coord2), np.array(nzhw)

    def __len__(self):
        # When training we need some small cubes without aneurysms.
        # So the total number of training is larger than the number of small cubes with aneurysms.
        if self.phase == 'train':
...
# Image enhancement for positive samples.      
...
# Class to extract positive samples.
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        for i in range(3):
            if not isRand:
                # Cut a cube near the positive sample.
                r = target[3] / 2
        ...
        pad.append([0,0])
        for i in range(3):
            leftpad = int(max(0,-start[i]))
            rightpad = int(max(0,start[i]+crop_size[i]-imgs.shape[i+1]))
...
# We get (xa,ya,za,da), which are the coordinates and scale for the anchor i from LabelMapping class.
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes, filename):
        ...
        label = -1 * np.ones(output_size + [len(anchors), 5])
        offset = ((stride.astype('float')) - 1) / 2
        oz = np.arange(offset, offset + stride * (output_size[0] - 1) + 1, stride)
        oh = np.arange(offset, offset + stride * (output_size[1] - 1) + 1, stride)
        ow = np.arange(offset, offset + stride * (output_size[2] - 1) + 1, stride)

        # Check if the bbox or anchor is too small to be a positive sample.
        for bbox in bboxes:
        ...
        if self.phase == 'train' and self.num_neg > 0:
            ...
            label[:, :, :, :, 0] = 0
            # Most labels are 0 and some selected negative samples are -1.
        ...
        if len(iz) == 0:
            ...
            idx = int(np.argmin(np.abs(np.log(target[3] / anchors))))
            pos.append(idx)
            flag = False
        else:
...
# Select samples based on a certain overlap threshold (th).
def select_samples(bbox, anchor, th, oz, oh, ow):
    ...
    if min_overlap > max_overlap:
        ...
        if np.isnan(s).sum() > 0:
            ...
            print("D",d)
            print("Anchor",anchor)
            print("Min", min_overlap)
            print("Max",max_overlap)
