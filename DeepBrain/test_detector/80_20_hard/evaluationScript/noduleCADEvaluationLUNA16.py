...
# plot settings
FROC_minX = 0.0078125#0.125 # Mininum value of x-axis of FROC curve
FROC_maxX = 2 # Maximum value of x-axis of FROC curve
bLogPlot = True
...
def computeFROC_bootstrap(FROCGTList,FROCProbList,FPDivisorList,FROCImList,excludeList,numberOfBootstrapSamples=1000, confidence = 0.95):
    ...
    # print("gt",FROCGTList)
    # print("prolist",FROCProbList)
    # print("FPDivisorList",FPDivisorList)
    print("FROCImList",len(FROCImList))
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    for seriesuid in seriesUIDs:
        ...
        for noduleAnnot in noduleAnnots:
            ...
            if diameter < 0.0:
              diameter = 10.0
            radiusSquared = pow((diameter), 2.0)
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        if bLogPlot:
            ...
            ax.xaxis.set_major_formatter(FixedFormatter([0.007,0.03,0.12,0.5,2]))
        
        # set your ticks manually
        ax.xaxis.set_ticks([0.0078125,0.03125,0.125,0.5,2])
        ax.yaxis.set_ticks(np.arange(0.2, 1, 0.1))
...
if __name__ == '__main__':
    annotations_filename = './dzh_annotations/annotations.csv'
    annotations_excluded_filename = './dzh_annotations/annotations_excluded.csv'
    seriesuids_filename = './dzh_annotations/seriesuids.csv'
    results_filename = './dzh_annotations/DeepBrain_FP_combine.csv'#3D Faster R-CNN - Res18.csv' #top5.csv'#
    noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,'./dzh_results')
    print("Finished!")