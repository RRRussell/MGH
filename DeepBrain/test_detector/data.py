...
class DataBowl3Detector(Dataset):
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            ...
            # print(np.max(label.reshape(-1,5)[:,0]))
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        if isScale:
            #print("crop,target",target)
            #print("crop,bboxes",bboxes)
            radiusLim = [8.,120.]
            scaleLim = [0.75,1.25]
        ...
        #print("in crop normstart:",normstart)
        #print("in crop normsize:",crop_size)
        ...
        coord = np.concatenate([xx[np.newaxis,...], yy[np.newaxis,...],zz[np.newaxis,:]],0).astype('float32')
        # print("in crop:",coord,coord.shape)
        pad = []
        pad.append([0,0])
        for i in range(3):
        ...
        # print("in crop start",start)
        for i in range(3):
            target[i] = target[i] - start[i] 
        for i in range(len(bboxes)):
            for j in range(3):
                bboxes[i][j] = bboxes[i][j] - start[j] 
        # print("in crop now target:",target)
        # print("in crop now bboxes:",bboxes)
...
def select_samples(bbox, anchor, th, oz, oh, ow):
    z, h, w, d = bbox
    #print("in select_samples: th:",th)
    #print("in select_samples: anchor:",anchor)
    #print("in select_samples: d:",d)
    max_overlap = min(d, anchor)
    min_overlap = np.power(max(d, anchor), 3) * th / max_overlap / max_overlap
    #print("in select_samples: max:",max_overlap)
    #print("in select_samples: min:",min_overlap)
    if min_overlap > max_overlap:
        return np.zeros((0,), np.int64), np.zeros((0,), np.int64), np.zeros((0,), np.int64)
    else:
        s = z - 0.5 * np.abs(d - anchor) - (max_overlap - min_overlap)
        e = z + 0.5 * np.abs(d - anchor) + (max_overlap - min_overlap)
        #print("in select_samples: s:",s)
        #print("in select_samples: e:",e)
        if np.isnan(s).sum() > 0:
        ...
        #print("in select_samples: mz:",mz)
        #print("in select_samples: iz:",iz)
        ...
        #print("in select_samples: s:",s)
        #print("in select_samples: e:",e)
        #print("in select_samples: mz:",mh)
        #print("in select_samples: iz:",ih)
        ...
        #print("in select_samples: s:",s)
        #print("in select_samples: e:",e)
        #print("in select_samples: mz:",mw)
        #print("in select_samples: iz:",iw)
        ...
        iw = iw.reshape((1, 1, -1))
        #print("in select_samples: iz:",iz)
        #print("in select_samples: ih:",ih)
        #print("in select_samples: iw:",iw)
        ...
        #print("in select_samples: centers:",centers)

        r0 = anchor / 2
        s0 = centers - r0
        e0 = centers + r0
        #print("in select_samples: r0:",r0)
        #print("in select_samples: s0:",s0)
        #print("in select_samples: e0:",e0)
        r1 = d / 2
        s1 = bbox[:3] - r1
        s1 = s1.reshape((1, -1))
        e1 = bbox[:3] + r1
        e1 = e1.reshape((1, -1))
        #print("in select_samples: r1:",r1)
        #print("in select_samples: s1:",s1)
        #print("in select_samples: e1:",e1)
        overlap = np.maximum(0, np.minimum(e0, e1) - np.maximum(s0, s1))
        
        #print("in select_samples: overlap:",overlap)
        
        intersection = overlap[:, 0] * overlap[:, 1] * overlap[:, 2]
        union = anchor * anchor * anchor + d * d * d - intersection

        iou = intersection / union
        #print("in select_samples: intersection:",intersection)
        #print("in select_samples: union:",union)
        #print("in select_samples: iou:",iou)
        mask = iou >= th
        #if th > 0.4:
         #   if np.sum(mask) == 0:
